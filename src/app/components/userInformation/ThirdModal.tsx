...
export default function ThirdModal({ userData }: ThirdModalProps) {
  return (
    <div className="text-center p-6 bg-white rounded-lg shadow-lg w-[500px] mx-auto">
      ...
      {/* Name */}
      <p className="text-lg">
        <strong className="text-sky-700">Name:</strong>{" "}
        {userData.name || <span className="text-gray-500">Not provided</span>}
      </p>

      {/* About Me */}
      <p className="text-lg mt-2">
        <strong className="text-sky-700">About Me:</strong>{" "}
        {userData.aboutMe || (
        ...
        <div className="flex flex-wrap gap-2 justify-center mt-2">
          {userData.selectedTags && userData.selectedTags.length > 0 ? (
            userData.selectedTags.map((tag, index) => (
              <span
                key={index}
                className="bg-sky-300 text-sky-900 px-3 py-1 rounded-full text-sm shadow"
              >
                {tag}
              </span>
            ))
      ...
      <p className="text-sky-600 text-sm mt-6">
        Remember - You can change any of this information in Profile tab or
        Settings
      </p>
    </div>
  );
